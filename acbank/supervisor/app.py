...
from src.services import ejecutar_supervisor, ejecutar_supervisor_stream
...
async def chat_endpoint(payload: ChatRequest):
    if not payload.message:
        return JSONResponse(status_code=400, content={"error": "Campo 'message' es obligatorio"})
    try:
...
async def agent_endpoint(input_data: RunAgentInput, request: Request):
    ...
    async def event_generator():
        ...
        # Stream events
        async for event in ejecutar_supervisor_stream(input_data):
